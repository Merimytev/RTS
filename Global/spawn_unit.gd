...
@export var builder_cost: int = 50
...
func _on_create_pressed_builder() -> void:
	...
	if Game.Energy >= builder_cost:
		...
		Game.Energy -= builder_cost  # Вычитаем стоимость
		worldPath.get_units()
	else:
		print("Недостаточно минералов для создания Builder. Требуется: ", builder_cost)
