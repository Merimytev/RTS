...
@onready var StatusLabel: Label = $Menu/Label 
...
func _show_end_screen(title_text: String) -> void:
	StatusLabel.text = title_text # Меняем текст на «Вы победили!» или «Вы проиграли!»
	get_tree().paused = true      # Останавливаем игру
	WinPanel.visible = true       # Показываем панель

#func _on_minerals_changed(val:int) -> void:
	#if not _won and val >= 500:
		#_won = true
		#_show_victory()
